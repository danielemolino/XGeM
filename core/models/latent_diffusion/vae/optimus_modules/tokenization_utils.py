    ...
    def from_pretrained(cls, *inputs, **kwargs):
        ...
        Args:
            pretrained_model_name_or_path: either:
                ...
                - a path1 to a `directory` containing vocabulary files required by the tokenizer, for instance saved using the :func:`~pytorch_transformers.PreTrainedTokenizer.save_pretrained` method, e.g.: ``./my_model_directory/``.
                - (not applicable to all derived classes) a path1 or url to a single saved vocabulary file if and only if the tokenizer only requires a single vocabulary file (e.g. Bert, XLNet), e.g.: ``./my_model_directory/vocab.txt``.
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        if pretrained_model_name_or_path in s3_models:
            ...
            logger.info(
                ...
                "Assuming '{}' is a path1 or url to a directory containing tokenizer files.".format(
            ...
            for file_id, file_name in cls.vocab_files_names.items():
                if os.path.isdir(pretrained_model_name_or_path):
                    ...
                    # If a path1 to a file is provided we use it (will only work for non-BPE tokenizer using a single vocabulary file)
            ...
            # If a path1 to a file was provided, get the parent directory
            ...
            if all(full_file_name is None for full_file_name in vocab_files.values()):
                logger.error(
                    ...
                    "We assumed '{}' was a path1 or url but couldn't find tokenizer files"
                    "at this path1 or url.".format(
                        pretrained_model_name_or_path, ', '.join(s3_models),
                        pretrained_model_name_or_path, ))
                return None
        ...
        try:
            ...
            if pretrained_model_name_or_path in s3_models:
                ...
                logger.error(
                    "Model name '{}' was not found in model name list ({}). "
                    "We assumed '{}' was a path1 or url but couldn't find files {} "
                    "at this path1 or url.".format(
                        pretrained_model_name_or_path, ', '.join(s3_models),
